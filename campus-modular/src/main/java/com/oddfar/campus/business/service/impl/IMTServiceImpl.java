...
import com.oddfar.campus.business.entity.ITravel;
import com.oddfar.campus.business.entity.IUser;
import com.oddfar.campus.business.mapper.IItemMapper;
import com.oddfar.campus.business.mapper.ITravelMapper;
...
@Service
public class IMTServiceImpl extends IMTServiceBase implements IMTService {
    ...
    @Resource
    private ITravelMapper iTravelMapper;
    @Resource
...
//    }
    ...
    public void refreshMTVersion() {
        String mtVersion = "";
        try {
            mtVersion = Convert.toStr(redisCache.getCacheObject("mt_version"));
            redisCache.deleteObject("mt_version");
            getMTVersion();
        } catch (Exception e) {
            logger.error("refreshMTVersion异常：{}", e.getMessage(), e);
            redisCache.setCacheObject("mt_version", mtVersion);
            logger.info("refreshMTVersion异常，使用原版本号：{}", mtVersion);
            pushPlusService.sendNotice("refreshVersion", e.getMessage());
        }
    }
...
//        request.header("User-Agent", "Mozilla/5.0 (Linux; Android 10; M2010J19SC Build/QKQ1.200830.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36");
//        request.header("Client-User-Agent", "android;29;Redmi;lime");
    ...
    public void reservation(IUser iUser) {
        ...
        for (String itemId : items) {
            ...
            try {
                ...
                logger.error("user：{},预约异常：{}", iUser.getRemark(), e.getMessage(), e);
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    logger.error("user：{}，获取耐力值异常：{}", iUser.getRemark(), e.getMessage(), e);
                    logContent += "执行报错--[申购耐力值]:" + e.getMessage();
                }
                //日志记录
                IMTLogFactory.reservation(iUser, logContent);
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        super.setRequest(request, iUser);

        HttpResponse execute = request.execute();
        String bodyStr = execute.body();
        logger.info("领取小茅运,uset->{},响应->{}", iUser.getRemark(), bodyStr);
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        logger.info("获取耐力值，user->{},响应->{}", iUser.getRemark(), body);
...
//        IMTLogFactory.reservation(iUser, logContent);
    ...
    public String travelReward(IUser iUser) {
        ...
        Map<String, Object> pageData = getUserIsolationPageData(iUser);
        Integer status = (Integer) pageData.get("status");
        if (status == 3) {
            BigDecimal currentPeriodCanConvertXmyNum = (BigDecimal) pageData.get("currentPeriodCanConvertXmyNum");
            BigDecimal travelRewardXmy = getXmTravelReward(iUser);
            if (travelRewardXmy.compareTo(new BigDecimal("0")) > 0) {
                super.sleep(3);
                // 获取小茅运
                receiveReward(iUser);
            }
            //本次旅行奖励领取后, 当月实际剩余旅行奖励
            if (travelRewardXmy.compareTo(currentPeriodCanConvertXmyNum) > 0) {
                String message = "当月无可领取奖励";
                logger.info("当月无可领取奖励，user->{}", iUser.getRemark());
                throw new ServiceException(message);
            }
        }

        Integer remainChance = (Integer) pageData.get("remainChance");
        if (remainChance < 1) {
            String message = "当日旅行次数已耗尽";
            logger.info("当日旅行次数已耗尽，user->{}", iUser.getRemark());
            throw new ServiceException(message);
        } else {
            SysConfigEntity config = new SysConfigEntity();
            config.setConfigKey("travel");
            SysConfigEntity retConfig = configMapper.selectConfig(config);
            if (retConfig != null && "0".equals(retConfig.getConfigValue())) {
                logger.info("配置为0，不行旅行");
                return null;
            }
            super.sleep(3);
            //小茅运旅行活动
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        super.setRequest(request, iUser);
        String body = request.execute().body();
        logger.info("开始旅行,user->{},响应->{}", iUser.getRemark(), body);
        JSONObject jsonObject = JSONObject.parseObject(body);
        if (jsonObject.getInteger("code") != 2000) {
            String message = "开始旅行失败：" + jsonObject.getString("message");
            logger.info("开始旅行失败，user->{}", iUser.getRemark());
            throw new ServiceException(message);
        }
    ...
    //查询 可获取小茅运
    public BigDecimal getXmTravelReward(IUser iUser) {
        //查询旅行奖励:
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        super.setRequest(request, iUser);
        String body = request.execute().body();
        logger.info("查询旅行奖励:user->{},响应->{}", iUser.getRemark(), body);
        JSONObject jsonObject = JSONObject.parseObject(body);
        if (jsonObject.getInteger("code") != 2000) {
            String message = jsonObject.getString("message");
            throw new ServiceException(message);
        ...
        JSONObject data = jsonObject.getJSONObject("data");
        BigDecimal travelRewardXmy = data.getBigDecimal("travelRewardXmy");
        logger.info("小茅运奖励:user->{},数量->{}", iUser.getRemark(), travelRewardXmy);
        if (travelRewardXmy.compareTo(new BigDecimal("0")) > 0) {
            try {
                ITravel iTravel = new ITravel();
                iTravel.setMobile(iUser.getMobile());
                iTravel.setUserId(iUser.getUserId());
                iTravel.setRemark(iUser.getRemark());
                iTravel.setXmyReward(travelRewardXmy);
                iTravel.setPostcardId(data.getString("postcardId"));
                iTravel.setPostcardName(data.getString("postcardName"));
                iTravelMapper.insert(iTravel);
            } catch (Exception e) {
                logger.error("保存旅行奖励记录失败，user->{}", iUser.getRemark(), e);
            }
        }
        //例如 1.95
    ...
    //获取用户页面数据
    public Map<String, Object> getUserIsolationPageData(IUser iUser) {
        //查询小茅运信息
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        logger.info("获取用户页面数据:user->{},响应->{}", iUser.getRemark(), body);
        JSONObject jsonObject = JSONObject.parseObject(body);
        if (jsonObject.getInteger("code") != 2000) {
            String message = jsonObject.getString("message");
            throw new ServiceException(message);
        ...
        Integer remainChance = xmTravel.getInteger("remainChance");
...
//        本月剩余旅行奖励
        BigDecimal exchangeRateInfo = getExchangeRateInfo(iUser);
        if (exchangeRateInfo.compareTo(new BigDecimal("0")) <= 0) {
            String message = "当月无可领取奖励";
            logger.info("当月无可领取奖励，user->{}", iUser.getRemark());
            throw new ServiceException(message);
        }
        ...
        if (status == 1) {
            if (energy < 100) {
                ...
                logger.info("耐力不足100，当前耐力值:{},user->{}", energy, iUser.getRemark());
                throw new ServiceException(message);
            }
        }
        // 进行中
        if (status == 2) {
            ...
            logger.info("旅行暂未结束，本次旅行结束时间:{},user->{}", formattedDate, iUser.getRemark());
            throw new ServiceException(message);
        }
    ...
    // 获取本月剩余奖励耐力值
    public BigDecimal getExchangeRateInfo(IUser iUser) {
...
//                .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
        ...
        super.setRequest(request, iUser);
        String body = request.form("__timestamp", DateUtil.currentSeconds()).execute().body();
        logger.info("本月剩余旅行奖励:user->{},响应->{}", iUser.getRemark(), body);
        JSONObject jsonObject = JSONObject.parseObject(body);
        if (jsonObject.getInteger("code") != 2000) {
            String message = jsonObject.getString("message");
            throw new ServiceException(message);
        ...
        //获取本月剩余奖励耐力值
        return jsonObject.getJSONObject("data").getBigDecimal("currentPeriodCanConvertXmyNum");
    ...
    public void reservationBatch() {
        ...
        for (IUser iUser : iUsers) {
            //延时
            super.sleep(10);
            logger.info("「开始预约用户」" + iUser.getRemark());
            //预约
            reservation(iUser);
...
//        List<IUser> iUsers = iUserService.selectReservationUser();
        ...
        for (IUser iUser : iUsers) {
            logger.info("「开始获得旅行奖励」" + iUser.getRemark());
...
//                        .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
                ...
                String body = request.execute().body();
                JSONObject jsonObject = JSONObject.parseObject(body);
                logger.info("查询申购结果: user->{},response->{}", iUser.getRemark(), body);
                if (jsonObject.getInteger("code") != 2000) {
                    String message = jsonObject.getString("message");
                    throw new ServiceException(message);
                }
                ...
                if(Objects.isNull(itemVOs) || itemVOs.isEmpty()){
                    logger.info("申购记录为空: user->{}", iUser.getRemark());
                    continue;
                }
                for (Object itemVO : itemVOs) {
                    ...
                    if (DateUtil.between(item.getDate("reservationTime"), new Date(), DateUnit.HOUR) < 24) {
                        logger.info("24小时内的订单：user->{},记录：{}", iUser.getRemark(), item.toJSONString());
                        String desc = item.getInteger("status") == 2 ? "成功 " : "失败 ";
                        String logContent = DateUtil.formatDate(item.getDate("reservationTime")) + " 申购" + desc + item.getString("itemName");
                        IMTLogFactory.reservation(iUser, logContent);
                        if (item.getInteger("status") == 2) {
                            String noticeContent = String.format("%s %s %s 成功", iUser.getRemark(), DateUtil.formatDate(item.getDate("reservationTime")), item.getString("itemId"));
                            pushPlusService.sendNotice(iUser.getRemark(), noticeContent);
                        }
                    }
                }
            } catch (Exception e) {
                logger.error("查询申购结果失败:user->{}，失败原因->{}", iUser.getRemark(), e.getMessage(), e);
            }
...
//        request.header("User-Agent", "iOS;16.3;Apple;?unrecognized?");
        ...
        JSONObject body = JSONObject.parseObject(execute.body());
        logger.info("user：{},预约结果：{}", iUser.getRemark(), body.toJSONString());
...
//                    .header("User-Agent", "iOS;16.3;Apple;?unrecognized?")
            ...
            super.setRequest(request, iUser);
            String body = request.execute().body();
            logger.info("查询小茅运&体力值:user->{},响应->{}",iUser.getRemark(), body);
    ...
    public void getRateAndPushPlus() {
        List<IUser> iUsers = iUserService.selectReservationUserOrderByMinute();
        IUser iUser = iUsers.get(0);
        try{
            long timestamp = System.currentTimeMillis();
            // 查询小茅运兑换比例
            String url = "https://h5.moutai519.com.cn/game/synthesize/exchangeRateInfo?__timestamp="+timestamp;
            HttpRequest request = HttpUtil.createRequest(Method.GET, url);
            request.header("MT-Device-ID", iUser.getDeviceId())
                    .header("MT-APP-Version", getMTVersion())
                    .cookie("MT-Token-Wap=" + iUser.getCookie() + ";MT-Device-ID-Wap=" + iUser.getDeviceId() + ";");
            super.setRequest(request, iUser);
            String body = request.execute().body();
            logger.info("查询小茅运兑换比例:user->{},响应->{}",iUser.getRemark(), body);
            JSONObject jsonObject = JSONObject.parseObject(body);
            if (jsonObject.getInteger("code") != 2000) {
                String message = jsonObject.getString("message");
                throw new ServiceException(message);
            ...
            JSONObject data = jsonObject.getJSONObject("data");
            Float exchangeRate = data.getFloat("exchangeRate");
            logger.info("小茅运兑换比例:{}", exchangeRate);
            pushPlusService.sendNotice("比例", exchangeRate.toString());
        }catch (Exception e){
            logger.error("查询小茅运兑换比例失败:user->{},失败原因->{}",iUser.getMobile(), e.getMessage());
        }
    }
